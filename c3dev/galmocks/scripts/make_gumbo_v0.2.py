"""Production script for gumbo_v0.2
"""
import os
import numpy as np
import argparse
...
from assign_elliptical_velocities import assign_ellipsoidal_velocities

TNG_LOGSM_CUT = 9.0
SEED = 43
OUTDRN = "/lcrc/project/halotools/C3EMC/gumbo"
UNIT_SIM_REDSHIFT_V0p2 = 0.592
...
if __name__ == "__main__":
    ...
    t0a = time()
    print("{0:.1f} seconds to load TNG host halos".format(t0a - t0))

    _tng_subhalos = load_tng_subhalos(args.tng_drn, args.tng_snapnum)
    t0b = time()
    print("{0:.1f} seconds to load TNG subhalos".format(t0b - t0a))
    ...
    nfw = NFWPhaseSpace(redshift=UNIT_SIM_REDSHIFT_V0p2)
    ...
    assign_ellipsoidal_velocities(
        ellipsoidal_key, output_mock, UNIT_SIM_REDSHIFT_V0p2, UNIT_LBOX
    )
    t11 = time()
    ...
    msk = output_mock["unit_halo_x"] < 100
    msk &= output_mock["unit_halo_y"] < 100
    msk &= output_mock["unit_halo_z"] < 100
    small_mock = output_mock[msk]
    outname = os.path.join(OUTDRN, args.outname.replace("gumbo", "small_gumbo"))
    small_mock.write(outname, path="data", overwrite=True)
